...
local lsps = {"clojure_lsp", "lua_ls", "jsonls", "yamlls", "marksman", "html", "basedpyright", "ts_ls", "terraformls", "tailwindcss", "dockerls", "docker_compose_language_service", "bashls", "taplo", "sqlls", "efm"}
...
local function _10_()
  ...
  local function _12_()
    return lspconfig.efm.setup({filetypes = {"clojure"}, settings = {rootMarkers = {".git/"}, languages = {clojure = {{rootMarkers = {"deps.edn", "project.clj"}, lintSource = "efm/background-check", lintCommand = "cat .background-check/errors.txt", lintIgnoreExitCode = true, lintFormats = {"%f:%l:%c %m"}, lintStdin = false}}}}})
  end
  local function _13_()
  ...
  return mlsp.setup_handlers({_11_, efm = _12_, tailwindcss = _13_})
end
local function _14_()
