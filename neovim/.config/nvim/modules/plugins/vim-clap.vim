...
nnoremap <leader>fg :Clap hidden_grep<cr>
...
" Adapted by Oliver Caldwell to enable hidden files in grep.
...
function! s:cmd(query) abort
  ...
  let cmd = 'rg -H --no-heading --vimgrep --hidden --smart-case "'.a:query.'"'.(has('win32') ? ' .' : '')
...
let g:clap#provider#hidden_grep# = s:grep
